...
with col2:
    p1 = st.number_input(label="proability 1 ", min_value=0.0, max_value=1.0, value=0.5)

col1, col2 = st.columns(2)
with col1: 
    prize2 = st.number_input(label="prize 2 ", value=0)
with col2:
    p2 = st.number_input(label="proability 2 ", min_value=0.0, max_value=1.0, value=0.5)

if p1 + p2 != 1.0:
    st.error(f"The sum of the probabilities must be 1: {p1} + {p2} = {p1 + p2}")
    ...
    st.write(f"""After playing  the lottery {num_trials} times: """)
    st.write(f"""The average payout is {np.average(payouts)}""")
    st.write(f"""The bar graph gives the number of times the each prize was won.""")
